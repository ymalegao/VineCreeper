  ...
  function setup() {
    ...
    for (let y = 0; y < totalStories; y++) {
      for (let x = 0; x < totalStories; x++) {
        // Tile the brick texture across the entire buildingGraphics canvas
        buildingGraphics.image(brickWall, x * storyWidth, y * storyHeight, storyWidth, storyHeight);
      }
    }
  ...
  function drawVine() {
    ...
    if (vine.segments[vine.segments.length - 1].y > 0) { // Stop when the vine reaches the top
      ...
      // Constrain the vine to the building bounds
      newX = constrain(newX, 0, storyWidth * totalStories - vineWidth / 2); // Keep within horizontal bounds
      newY = constrain(newY, 0, storyHeight * totalStories - vineHeight / 2); // Keep within vertical bounds
